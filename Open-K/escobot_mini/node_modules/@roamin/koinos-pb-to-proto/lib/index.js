"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.convert = void 0;
const descriptor_pb_js_1 = require("google-protobuf/google/protobuf/descriptor_pb.js");
const util_1 = require("./util");
/**
  * Convert a binary proto file into a text proto file
  * @returns ProtoDescriptor
  * @example
  * ```js
  * const pb = '';
  *
  * const protos = koinosPbToProto.convert(pb);
  *
  * for (const proto of protos) {
  *   console.log(proto.file)
  *   console.log(proto.definition);
  * }
  * ```
  */
const convert = (pb) => {
    const input = typeof pb === 'string' ? Buffer.from(pb, 'base64') : pb;
    const fds = descriptor_pb_js_1.FileDescriptorSet.deserializeBinary(input);
    const protoDescriptors = [];
    for (const fd of fds.getFileList()) {
        const file = fd.getName() || '';
        let definition = '';
        // syntax
        if (fd.hasSyntax()) {
            definition += (0, util_1.getSyntax)(fd);
        }
        // package
        if (fd.hasPackage()) {
            definition += (0, util_1.getPackage)(fd);
        }
        // options
        if (fd.hasOptions()) {
            // do nothing with options for now
        }
        // imports
        for (const dep of fd.getDependencyList()) {
            definition += (0, util_1.getImport)(dep);
        }
        // enums
        for (const enumDesc of fd.getEnumTypeList()) {
            definition += (0, util_1.getEnum)(enumDesc);
        }
        // extensions
        for (const extDesc of fd.getExtensionList()) {
            definition += (0, util_1.getExtension)(extDesc);
        }
        // messages
        for (const messageDesc of fd.getMessageTypeList()) {
            definition += (0, util_1.getMessage)(messageDesc);
        }
        protoDescriptors.push({ file, definition });
    }
    return protoDescriptors;
};
exports.convert = convert;
